...
public class FirstServiceimpl implements IFirstService {
	...
	public Person execute1(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			name = name.isEmpty() ? "홍길동" : name;	// 사용자가 입력한 name이 업으면 빈 문자열이 전달된다.
			
			String strAge = request.getParameter("age");
			strAge = strAge.isEmpty() ? "0" : strAge;	// 사용자가 입력한 age이 업으면 빈 문자열이 전달된다.	
			int age = Integer.parseInt(strAge);
			
			// 0~ 100 범위를 벗어난 경우 예외 발생시키디
			if(age < 0 || age > 100) {
				throw new RuntimeException(age + " 살은 불가능");
			}
