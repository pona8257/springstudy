...
public class EmployeeListServiceImpl implements EmployeeListService {
	...
	public void getEmployeeListUsingPagination(HttpServletRequest request, Model model) {
		...
		// 세션에 있는 recordPerPage를 가져온다. 세션에 없는 경우 recordPerPage=10으로 처리한다. 세션은 파라미터 받을때 object값을 받는다
		...
		// PageUtil(Pagenation에 필요한 모든 정보) 계산하기
		...
		switch(order) {
		case "ASC": model.addAttribute("order", "DESC"); break;		// 현재 ASC 정렬 이므로 다으 정렬은 DESC이라고 Jsp에 알려준다.
