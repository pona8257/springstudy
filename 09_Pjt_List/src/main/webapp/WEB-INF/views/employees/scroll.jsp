...
<script>
	...
	function fnGetEmployees(){
		...
		$.ajax({
			...
			// 응답
			dataType: 'json',
			success: function(resData){	//resData = {"employees": [{}, {}, ... ], "totalPage": 12} 한 사람당 객체 하나씩
				// 전체 페이지 개수를 전역 변수 totalPage에 저장
				totalPage = resData.totalPage;
				// 스크롤을 통해서 목록을 9개씩 가져올때 마다 page가 증가해야 한다.
				page++;	
				// 화면에 뿌리기
				//$.each(배열, function(인덱스, 요소){})
				$.each(resData.employees, function(i, employee){		// employee는 "employees": [{}, {}, ... ] 에서  {}이다
	...
	$(window).on('scroll', function(){
		...
		//timerId = setTimeout(function(){}, 500);	// 500밀리초(0.5)초 후에 function() 수행
