...
/*
 	@Controller
 	@Component를 포함하고 있어서 자동으로 Spring Container 에 Bean으로 등록되지.
 	나는 스프링에 의해서 사용되고 있어
 */
...
public class MyController {
	...
	/*
	 	...
	 	2. 메소드명 : 아무일도 안한다. 의미 없다
	 	3. 매개변수 : 요청에 응답에 따라 다르다. (요청이 필요한 경우 HttpServletRequest, 응답이 필요하면 HttpResponse 등)
	 */
	...
	/*
	 	...
	 	@RequestMapping(value="/list.do")							 GET 방식의 method는 생략 가능
	 	@RequestMapping("/list.do")									 value 속성만 작성하는 경우에는 값만 작성한다.
	 */
	
	@RequestMapping("/list.do")
	public String list() {
		return "board/list";	// 실제 퍼리되는 경로 : /WEB-INF/views/board/list.jsp
		/*
		 	return "/board/list" ;	슬래시(/)로 시작한 Jsp 경로
		 	실제 처리되는 경로 : /WEB-INF/views//board/list.jsp
		 	하지만, 실제로는 /WEB-INF/views/board/list.jsp 경로롤 처리된다
		 */
	}
