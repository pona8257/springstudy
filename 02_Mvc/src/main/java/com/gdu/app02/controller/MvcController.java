...
public class MvcController {
	...
	/*
	 	...
	 	메소드마다 request.setCharacterEncoding("UTF-8")을 작성하는 것은 매우 비효율 적이므로,
	...
	public String detail(HttpServletRequest request, Model model) {
		...
		/*
		 	...
		 	3. forward할 데이터를 Model의 addAtrribute() 메소드로 저장한다.
		 */
	...
	/*
	 	2. @RequestParam으로 요청 파라미터 처리하기
	 		...
	 		3) defaultValue : 용청 파라미터가 없을 때 대신 사용할 값
	 */
	@GetMapping("/detail.me")																				// 최대한 줄이는 방법은 String name, int age, Model model로 줄여서 @RequestParam처리 안해도 되긴된다(그대신 디폴트 처리는 못함)
	public String detailMe(@RequestParam(value="name", required=false, defaultValue="홍길동") String name,	// 요청 파라미터 name이 없으면 "홍길동"을 사용한다. 
						   @RequestParam(value="age", required=false, defaultValue="0") int age,			// Integer.Parsint 안해도 된다 Spring이 해준다!! 요청 파라미터 age이 없으면 "0"을 사용한다. 
						   Model model) {
		model.addAttribute("name", name);
		model.addAttribute("age", age);
	...
	*/
	...
	public String detailGdu(Person p) {	// name과 age를 필드로 가진 커맨드 객체 Person p
										...
										// 객체 타입인 Person을 사용한다. 
										// Model에 저장되는 속성명은 객체 타입 Person으로 수정해서 사용한다.
		return "mvc/detail";
	}
	/*
	@GetMapping("/detail.gdu")
