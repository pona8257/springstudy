...
<script>
  ...
  function fnChekId(){
	  $('#id').on('keyup', function(){
		  ...
		  $.ajax({
			  ...
			  data: 'id=' + id,
			  dataType: 'json',
			  success: function(resData){  // resData = {"existId: true"} 또는 {"existId": false}
  ...
  function fnCheckPw(){
	  ...
	  $('#pw').on('keyup', function(){
		  ...
		  // 길이 및 정규식 (4~20자, 소문자+대문자+특수문자 사용 가능, 3개 이상 조합)
		  let pwLength = pw.length;
		  let validCount = /[a-z]/.test(pw)           //   소문자를 가지고 있으면 true(1), 없으면 false(0)
		                 + /[A-Z]/.test(pw)           //   대문자를 가지고 있는가 true(1), 없으면 false(0)
		                 + /[0-9]/.test(pw)           //     숫자를 가지고 있는가 true(1), 없으면 false(0)
		                 + /[^a-zA-Z0-9]/.test(pw);    // 특수문자를 가지고 있는가 true(1), 없으면 false(0)
		                 
		  verifyPw = (pwLength >= 4) && (pwLength <= 20) && (validCount >= 3);
		  id(verifyPw){
			  $('#msgPw').text('사용 가능한 비밀번호입니다.');
		  } else{
			  $('#msgPw').text('4~20자, 소문자+대문자+특수문자 사용 가능, 3개 이상 조합');
		  }
	  })
  }
  ...
  function fnCheckEmail(){
	  ...
	  $('#btnGetCode').on('click', function(){
		  ...
		  new Promise(function(resolve, reject){
			  ...
			  // 정규식
			  let reqEmail = /^[a-zA-Z0-9-_]+@[a-zA-Z0-9]{2,}(\.[a-zA-Z]{2,6}){1,2}$/;
			  //                  gt_mon     @ naver         (.com)/(.co) ~ 
			  ...
			  data: 'email=' + email,
			  dataType: 'json',
			  success: function(resData){  // resData = {"enableEmail": true} 또는 {"enableEmail": flase}
				  if(resData.enableEmail){
  				  resolve();  // then 메소드에 정의된 function을 호출한다.
				  } else{
   				  reject(2);  // catch 메소드에 정의된 function을 호출한다. 인수로 2를 전달한다
				  }
			  }
		  }).then(function(){
		       ...
		       $.ajax({
		         ...
		         success: function(resData){ // resData = {"authCode" : "6T43G9"} 사용자에게 전송한 인증코드를 의미
		           ...
		           $('#btnVerifyCode').on('click', function(){
		             ...
		             verifyEmail = resData.authCode == $('#authCode').val(); // 사용자에게 전송한 입력코드 == 사용자가 입력한 인증코드
		             if(verifyEmail){ 
		               alert('인증되었습니다.');    
		             } else{
		               alert('인증에 실패했습니다.');
		       ...
		       case 1:
		         ...
		         break;
		       case 2:
		         msg = '이미 사용중인 이메일입니다.';   // 이메일 중복체크 실패
		         break;
		       }
		       $('#msgEmail').text(msg);
	  ...
	  }).catch(function(number){ // reject() 함수를 호출할 때 처리되는 함수(ajax 처리 error 했을 때)
