...
-- 좋아요 외래키(블로그) : 블로그가 삭제되면 좋아요도 삭제된다.3
ALTER TABLE GOOD_T
    ADD CONSTRAINT FK_COOD_BLOG
        FOREIGN KEY(BLOG_NO) REFERENCES BLOG_T(BLOG_NO)
            ON DELETE CASCADE;

-- 좋아요 외래키(회원) : 좋아요를 누른 사람이 삭제되면 좋아요도 삭제된다.4
ALTER TABLE GOOD_T
    ADD CONSTRAINT FK_COOD_MEMBER
        FOREIGN KEY(MEMBER_NO) REFERENCES MEMBER_T(MEMBER_NO)
            ON DELETE CASCADE;
